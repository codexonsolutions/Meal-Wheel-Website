...
type ApiOrder = {
  ...
  deliveryAddress?: { street: string; city: string; postal: string; notes?: string };
  items: ApiOrderItem[];
  subtotal: number;
  gstTotal: number;
  deliveryFee: number;
...
function StatusBadge({ status }: { status: ApiOrder["status"] }) {
  const map: Record<ApiOrder["status"], { label: string; tone: string }> = {
    ...
    preparing: { label: "Preparing", tone: "bg-blue-50 text-blue-700 border-blue-200" },
    "out-for-delivery": { label: "Out for delivery", tone: "bg-purple-50 text-purple-700 border-purple-200" },
    delivered: { label: "Delivered", tone: "bg-emerald-50 text-emerald-700 border-emerald-200" },
    cancelled: { label: "Cancelled", tone: "bg-rose-50 text-rose-700 border-rose-200" },
  };
  const conf = map[status];
  return <span className={`inline-flex items-center rounded-full border px-2.5 py-1 text-xs font-medium ${conf.tone}`}>{conf.label}</span>;
}
  ...
  return (
    <div className="w-full">
      <div className="relative flex items-center justify-between">
        ...
        {/* Progress */}
        <div
          className="absolute left-0 h-1.5 bg-primary rounded-full transition-all"
          style={{
            width: `${(Math.max(activeIndex, 0) / (STATUS_STEPS.length - 1)) * 100}%`,
          }}
        />
            ...
            <div key={step.key} className="relative z-10 flex flex-col items-center w-1/4">
              <div
                className={`flex h-8 w-8 items-center justify-center rounded-full border text-xs font-bold transition-colors ${
                  done ? "bg-primary text-white border-primary" : "bg-white text-gray-500 border-gray-300"
                }`}
              >
                <Icon className="size-4" />
              </div>
              <div className={`mt-2 text-xs ${done ? "text-foreground" : "text-muted-foreground"}`}>
                {step.label}
              </div>
            </div>
          );
...
export default function Page() {
  ...
  useEffect(() => {
    ...
    async function fetchOrder(signal?: AbortSignal) {
      ...
      try {
        const res = await fetch(`${base}/orders/${id}`, { cache: "no-store", signal });
        if (!res.ok) {
    ...
    timer = window.setInterval(() => fetchOrder(ctrl.signal), 20000) as unknown as number;
    return () => {
      ctrl.abort();
  ...
  const restaurantName = order?.restaurant?.name || (order?.restaurants?.[0]?.name ?? "");
          ...
          {loading ? (
            <Card>
              <CardHeader className="border-b">
                ...
                <CardDescription>Fetching the latest order details</CardDescription>
              </CardHeader>
              <CardContent>
                ...
                <Card>
                  ...
                  <CardContent className="py-6 space-y-4">
                    {order.items.map((it, idx) => (
                      <div key={`${it.itemId}-${idx}`} className="flex items-start gap-4">
                        <div className="relative h-16 w-16 rounded-lg overflow-hidden border bg-gray-50">
                          <SafeImage src={it.imageUrl} alt={it.name} fill className="object-cover" />
                        </div>
                        <div className="flex-1">
                          <div className="flex items-start justify-between gap-2">
                            <div>
                              <div className="font-medium text-foreground leading-tight">{it.name}</div>
                  ...
                  <Card>
                    ...
                    <CardContent className="py-6 space-y-1 text-sm">
                      ...
                      {order.deliveryAddress?.notes ? (
                        <div className="text-muted-foreground text-xs mt-2">Notes: {order.deliveryAddress?.notes}</div>
                      ) : null}
                    </CardContent>
                  </Card>
                </div>
                ...
                <Card>
                  ...
                  <CardContent className="py-6 space-y-3">
                    ...
                    <div className="flex items-center justify-between text-sm">
                      ...
                      <span className="font-medium">{currency(order.gstTotal)}</span>
                    </div>
                    <div className="flex items-center justify-between text-sm">
                      <span className="text-muted-foreground">Delivery Fee</span>
                      <span className="font-medium">{currency(order.deliveryFee)}</span>
                    </div>
                    <div className="border-t pt-3 flex items-center justify-between text-base font-semibold">
                      <span>Total</span>
                      <span className="text-secondary">{currency(order.total)}</span>
                    </div>
