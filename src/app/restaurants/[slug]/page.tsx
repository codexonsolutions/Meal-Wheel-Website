  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        const restRes = await fetch(`${base}/restaurant`, {
          cache: "no-store",
        });
        if (!restRes.ok) throw new Error("Failed to fetch restaurants");
        const restData = await restRes.json();
        const list: RestaurantApi[] = Array.isArray(restData.restaurants)
        ...
        const itemsRes = await fetch(`${base}/items/restaurant/${match._id}`, {
          cache: "no-store",
        });
        if (!itemsRes.ok) throw new Error("Failed to fetch items");
        const itemsData = await itemsRes.json();
        const itemsList: ItemApi[] = Array.isArray(itemsData.items)
          ? itemsData.items
          : [];
        if (active) {
          setItems(itemsList);
        ...
        const msg =
          e instanceof Error ? e.message : "Failed to load restaurant";
